<!-- Add or Edit Engineer Modal -->
<?php
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT * from `engineers_list` where id = '{$_GET['id']}' ");
    if($qry->num_rows > 0){
...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Update ": "Create New " ?> Engineer</h3>
	</div>
	<div class="card-body">
		<form action="" id="engineer-form">
			...
			<div class="form-group">
                ...
                <input name="role" id="role" type="text" class="form-control rounded-0" value="<?php echo isset($role) ? $role : ''; ?>" required>
			</div>
			
            <div class="form-group">
				<label for="status" class="control-label">Task</label>
                <select name="task" id="task" class="custom-select selevt">
                <option value="1" <?php echo isset($task) && $task == 1 ? 'selected' : '' ?>>Service</option>
                <option value="0" <?php echo isset($task) && $task == 0 ? 'selected' : '' ?>>Repair</option>
                </select>
			</div>
			<div class="form-group">
...
<script>
	...
	$(document).ready(function(){
		...
		$('#engineer-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_engineer",
