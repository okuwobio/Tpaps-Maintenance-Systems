<?php
// all add and edit and delete function
...
Class Master extends DBConnection {
	...
	function save_engineer(){
		...
		$check = $this->conn->query("SELECT * FROM `engineers_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Enginner already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `engineers_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `engineers_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Engineer successfully saved.");
			else
				$this->settings->set_flashdata('success',"Engineer successfully updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_engineer(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `engineers_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Engineer successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_aircraft(){
		...
		$check = $this->conn->query("SELECT * FROM `aircrafts_list` where `name` = '{$name}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Aircraft already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `aircrafts_list` set {$data} ";
			$save = $this->conn->query($sql);
		}else{
			$sql = "UPDATE `aircrafts_list` set {$data} where id = '{$id}' ";
			$save = $this->conn->query($sql);
		}
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"New Aircraft successfully saved.");
			else
				$this->settings->set_flashdata('success',"Aircraft successfully updated.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_aircraft(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `aircrafts_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Aircraft successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_request(){
		...
		if($save){
			...
			$save = $this->conn->query($sql);
			$statusToFill = ($_POST['status'] == 3) ? 0 : 1;
			$sql = "UPDATE engineers_list SET `status` = '$statusToFill' WHERE id = '$_POST[mechanic_id]'";
			$save = $this->conn->query($sql);
			if($save){
				$resp['status'] = 'success';
				$resp['id'] = $rid;
...
switch ($action) {
	...
	break;
	case 'save_engineer':
		echo $Master->save_engineer();
	break;
	case 'delete_engineer':
		echo $Master->delete_engineer();
	break;
	case 'save_aircraft':
		echo $Master->save_aircraft();
	break;
	case 'delete_aircraft':
		echo $Master->delete_aircraft();
	break;
	case 'save_request':
